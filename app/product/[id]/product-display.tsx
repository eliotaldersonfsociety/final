...
interface ProductDisplayProps {
  product: Product;
  initialOffers?: Product[];  // Nueva prop para ofertas
}
...
export default function ProductDisplay({ product, initialOffers = [] }: ProductDisplayProps) {
  ...
  const handleAddToCart = () => {
    ...
    if (
      product.range &&
      product.range.min !== null &&
      product.range.max !== null &&
...
const OffersSlider = () => {
  ...
  // OPCIÓN 1: Limitar a solo 3 slides
  const limitedOffers = initialOffers.slice(0, 3);
  
  // OPCIÓN 2: Mostrar paginación solo si hay más de 4 productos
  const showPagination = initialOffers.length > 4;
  
  return (
    <div className="mt-12 bg-gray-50 p-6 rounded-lg">
      <h3 className="text-xl font-bold mb-6">Ofertas especiales</h3>
      <div className="relative">
        <Swiper
          ...
          loop={limitedOffers.length > 2} // Solo loop si hay más de 2 productos
          pagination={showPagination ? { // Solo mostrar paginación si hay muchos productos
            clickable: true,
          } : false}
          breakpoints={{
            640: {
          ...
          className="py-4"
        >
          {/* USAR limitedOffers en lugar de initialOffers */}
          {limitedOffers.map((offer) => (
            <SwiperSlide key={offer.id}>
              <div className="border rounded-lg overflow-hidden hover:shadow-lg transition-all bg-white h-full">
                <div className="relative aspect-square bg-gray-100">
                  <Image
                    loader={customLoader}
                    src={offer.images?.[0] || '/placeholder-product.png'}
                    alt={offer.title}
                    fill
                    className="object-contain p-2"
                    sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, 25vw"
                  />
                  {offer.compare && offer.compare > offer.price && (
                    <Badge className="absolute top-2 left-2 bg-red-500 hover:bg-red-600">
                      -{Math.round((1 - Number(offer.price)/Number(offer.compare)) * 100)}%
                    </Badge>
                  )}
                </div>
                <div className="p-3">
                  <h4 className="font-medium text-sm line-clamp-2 h-10">{offer.title}</h4>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {offer.compare && offer.compare > offer.price && (
                      <p className="text-xs text-gray-500 line-through">
                        ${Number(offer.compare).toFixed(2)}
                      </p>
                    )}
                  </div>
                  <Button 
                    size="sm"
                    className="w-full mt-2 text-xs bg-red-800 hover:bg-red-900 text-white"
                    onClick={() => addToCart({
                      id: offer.id,
                      name: offer.title,
                      price: Number(offer.price),
                      image: offer.images?.[0] || '/placeholder-product.png',
                      quantity: 1
                    })}
                  >
...
{/* Selector de tallas numéricas - Versión corregida */}
{(product.range !== null && 
  product.range.min !== null && 
  product.range.max !== null && 
  product.range.min < product.range.max && // Cambiado a < para evitar rango de 0-0
  product.category !== "Moda") ? (
  ...
  <div className="space-y-2">
    ...
    >
      {product.range &&
        Array.from(
          { length: product.range.max - product.range.min + 1 },
          (_, i) => {
            const size = product.range!.min + i;
            return (
              <Label
                key={size}
