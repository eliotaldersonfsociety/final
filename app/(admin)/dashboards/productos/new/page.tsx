...
import { compressImage } from '@/app/helpers/imageHelper'; // Asegúrate de que la ruta sea correcta
...
const handleGenerateImage = async () => {
  ...
  try {
    ...
    // Aquí puedes agregar lógica adicional si necesitas optimizar las imágenes generadas
    setProduct((prev) => ({
      ...prev,
  ...
  if (!isNaN(parsedValue)) {
    setProduct((prev) => {
      const currentRange = prev.range || { min: 18, max: 45 };
      const newRange = {
        ...currentRange,
        [name]: parsedValue,
      };
  ...
  const handleSubmit = async () => {
    if (!product.title || !product.description || !product.price || !product.category || !product.landingData) {
      toast({
    ...
    try {
      const payload = {
        ...
        size_range: useSizeRange && product.range ? JSON.stringify(product.range) : null,
  ...
  const handleLandingDataGenerated = (data: LandingData) => {
    console.log('Landing data generated:', data);
                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              id="vendor"
                              name="vendor_display"
                              value="Colombia Importadora"
                              disabled
                              className="cursor-not-allowed bg-gray-100 text-gray-700"
                            />

                            {/* Oculto, se envía en el submit */}
                            <input type="hidden" name="vendor" value="Colombia Importadora" />
                          </div>
                        </CardContent>
                        <CardContent>
                        ...
                        <CardContent className="space-y-6">
                          ...
                          <div className="space-y-3">
                            <div className="flex items-center space-x-2 mb-2">
                              <Switch
                                id="useSizeRange"
                                checked={useSizeRange}
                                onCheckedChange={(checked) => {
                                  setUseSizeRange(checked);
                                  setProduct((prev) => ({
                                    ...prev,
                                    range: checked ? (prev.range ?? { min: 18, max: 45 }) : null,
                                  }));
                                }}
                              />
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value="hogaryjardin">Hogar y Jardin</SelectItem>
                  ...
                  <Card className="mt-6">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm">
                          {product.range?.min ?? 18} - {product.range?.max ?? 45} cm
                        </p>
                      </div>
                      <div className="space-y-2">
